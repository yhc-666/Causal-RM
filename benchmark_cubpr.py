...
def train(model, train_data, optimizer, num_epochs, val_data, patience, args):
    """
    Global pairwise CUBPR (clipped unbiased BPR) adapted from old recommender baseline:
    ...
    where pi is clipped to [clip_min, 1] (stronger clipping than UBPR).
    """
    if not args.is_training:
        return
    if not args.binary:
        raise NotImplementedError("Global CUBPR baseline currently supports binary (0/1) labels only.")
...
def main():
    ...
    print("=" * 70)
    print("Global Pairwise CUBPR Reward Modeling")
    print("=" * 70)
