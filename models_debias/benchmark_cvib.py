...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/cvib/{pre_args.data_name}",
        ...
        "w_info": 0.2,  # Info weight
        "w_entropy": 0.2,  # Entropy weight
        "rerun": False,
        "monitor_on": "train",
        "binary": True,
        "use_tqdm": True,
    ...
    parser.add_argument("--w_info", type=float, help="Info weight") # key parameter, tuning w_info and w_entropy to zero immediately yields the naive method.
    parser.add_argument("--w_entropy", type=float, help="Entropy weight") # key parameter
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    """
    Train reward model using CVIB loss.

    Code link: https://github.com/RyanWangZf/CVIB-Rec/blob/master/matrix_factorization.py
    """
    ...
    for epoch in range(num_epochs):
        ...
        bar = tqdm(train_loader, desc=f"Training CVIB Model Epoch {epoch + 1}/{num_epochs}", leave=False) if args.use_tqdm else train_loader
        for batch_X, batch_y in bar:
            ...
            # CVIB loss
            reward_pred_scaled = F.sigmoid(reward_pred)
            pos_mask = batch_y > 0.5
            reward_pred_sub = reward_pred_scaled[pos_mask] if pos_mask.any() else reward_pred_scaled
            reward_pred_sub_avg = reward_pred_sub.mean()
            reward_pred_avg = reward_pred_scaled.mean()
            infoloss = -1 * (
                reward_pred_sub_avg * reward_pred_avg.log() + 
                (1 - reward_pred_sub_avg) * (1 - reward_pred_avg).log()
            )
            entropyloss = torch.mean(reward_pred_sub * reward_pred_sub.log()) # maximize entropy

            # Apply task weight
            weighted_loss =  args.w_reg * loss + args.w_info * infoloss + args.w_entropy * entropyloss
...
def main():
    ...
    print("="*70)
    print("CVIB Reward Modeling (PU setting: UNK->0, mask-invisible)")
    print("="*70)
