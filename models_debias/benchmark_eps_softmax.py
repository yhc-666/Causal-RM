...
from tools.losses import ECELoss, MAELoss, ECEandMAE
...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/eps_softmax/{pre_args.data_name}",
    ...
    parser.add_argument("--alp", type=float, help="Alpha parameter for epsilon-softmax")
    parser.add_argument("--beta", type=float, help="Beta parameter for epsilon-softmax")
    parser.add_argument("--eps", type=float, help="Beta parameter for epsilon-softmax")
    parser.add_argument("--m", type=float, help="M parameter for epsilon-softmax")
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    ...
    criterion = ECEandMAE(args.m, alpha=args.alp, beta=args.beta, eps=args.eps, reduction='none')
