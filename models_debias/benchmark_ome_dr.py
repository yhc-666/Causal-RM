...
def train_baseline_model(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    ...
    PU setting (mask-invisible):
      - Train a baseline reward model on the full dataset with y_train_binary (UNK->0).

    This baseline is used in the DR correction term.
    """
    if not args.is_training: return
...
def train(model, noise_pred_model, baseline_model, train_loader, train_full_iter, optimizer, num_epochs, val_data, patience, args):
    if not args.is_training: return
...
def main():
    ...
    print("="*70)
    print("OME DR Reward Modeling (PU setting: UNK->0, mask-invisible)")
    print("="*70)
    ...
    # Step 2: Train baseline reward model on full PU dataset (UNK->0)
    print("\n" + "="*70)
    ...
    # Step 3: Train final reward model using OME DR
    print("\n" + "="*70)
    print("Step 3: Training Reward Model with OME DR")
    print("="*70)
