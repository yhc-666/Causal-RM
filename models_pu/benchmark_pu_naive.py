...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/pu_naive/{pre_args.data_name}",
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    """
    Train reward model using PU-naive loss:
    L = mean(loss on y=1 samples) + mean(loss on y=0 samples)

    This gives equal weight to both classes regardless of class imbalance.

    In PU learning context:
        - y=1: observed positive samples
        - y=0: observed negative + unobserved samples (treated as unlabeled/negative)
    """
    ...
    for epoch in range(num_epochs):
        ...
        bar = tqdm(train_loader, desc=f"Training PU-Naive Model Epoch {epoch + 1}/{num_epochs}", leave=False) if args.use_tqdm else train_loader
        for batch_X, batch_y, batch_mask in bar:
            ...
            # Compute loss on positive samples (loss_pp)
            if pos_mask.sum() > 0:
                loss_pp = criterion(reward_pred[pos_mask], batch_y[pos_mask])
                loss_pp = loss_pp.mean()
            else:
                loss_pp = torch.tensor(0.0, device=batch_X.device)

            # Compute loss on unlabeled/negative samples (loss_un)
            if neg_mask.sum() > 0:
                loss_un = criterion(reward_pred[neg_mask], batch_y[neg_mask])
                loss_un = loss_un.mean()
            else:
                loss_un = torch.tensor(0.0, device=batch_X.device)

            # PU-naive loss: equal weight to both classes
            loss = loss_pp + loss_un
...
def main():
    ...
    print("="*70)
    print("PU-Naive Reward Modeling")
    print("="*70)
    ...
    # For PU-naive: use ALL samples, not just observed
    ...
    print(f"  - Negative/Unlabeled samples (y=0): {(y_train == 0).sum().item()}")
    ...
    # Train reward model on all data (PU-naive approach)
    print("\n" + "="*70)
    print("Step 1: Training PU-Naive Reward Model")
    print("="*70)
    ...
    # Train metrics on observed samples; eval metrics on full PU labels (mask-blind).
