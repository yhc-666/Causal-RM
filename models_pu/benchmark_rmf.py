...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/rmf/{pre_args.data_name}",
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    """
    Train reward model using RMF-style loss adapted from recommender-system RMF.

    Loss (binary case, with p = sigmoid(logit)):
      L = sum_i [ (y_i/pi_i) * (1 - p_i)^2 + (1 - y_i/pi_i) * p_i^2 ] / sum_i [ y_i + (1 - y_i) ]

    where pi_i is clipped to [1e-8, 1] (or args.clip_min).
    """
    ...
    for epoch in range(num_epochs):
        ...
        bar = (
            tqdm(train_loader, desc=f"Training RMF Model Epoch {epoch + 1}/{num_epochs}", leave=False)
            if args.use_tqdm
            else train_loader
        )
...
def main():
    ...
    print("\n" + "=" * 70)
    print("Step 1: Training RMF Reward Model")
    print("=" * 70)
