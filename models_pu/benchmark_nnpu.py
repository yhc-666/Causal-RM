...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/nnpu/{pre_args.data_name}",
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    ...
    for epoch in range(num_epochs):
        ...
        bar = tqdm(train_loader, desc=f"Training NNPU Model Epoch {epoch + 1}/{num_epochs}", leave=False) if args.use_tqdm else train_loader
...
def main():
    ...
    print("="*70)
    print("Non-Negative PU (NNPU) Learning Reward Modeling")
    ...
    # For NNPU learning: use ALL samples, not just observed
    ...
    # Train reward model on all data (NNPU learning approach)
    print("\n" + "="*70)
    print("Step 1: Training NNPU Reward Model")
    print("="*70)
