...
def parse_arguments():
    ...
    parser.add_argument("--num_neg", type=int, help="Number of j samples per i (user-wise)")
    ...
    parser.add_argument("--binary", type=str2bool, help="Whether to use binary labels (must be True for UPRL)")
...
def _precompute_pairs_uprl(user_id_np, y_np, num_neg, seed):
    ...
    for i in pos_idx:
        ...
        cand = group[group != i]  # "all" (incl. positives) excluding itself
        if cand.size > 0:
...
def train(model, train_data, optimizer, num_epochs, val_data, patience, args):
    """
    User-wise UPRL (binary only), adapted from old recommender UPRL:

      - i sampled from positives (y=1)
      - j sampled from same user (all samples, y in {0,1}, j != i)
      - if a user has no within-user j candidates, fallback: sample j from other users (no explicit switch)

    Loss follows the original structure:
      pairwise term:
        base = -log σ(s_pair(i) - s_pair(j))
        w = (1/pi_i) * (1 - y_j) / pi_j
        corr = (pi_j * (1 - q)) / (1 - q * pi_j)
        pair_loss = mean( clip(corr * w * base, 0, 1e2) )

      pointwise IPS-like term (on j samples):
        y_over_p = y_j / pi_j
        ll = y_over_p * log p + (1 - y_over_p) * log(1 - p)
        point_ce = -mean( clip(ll, -1e8, 1e8) )

    where:
      p = sigmoid(s_point(j)), q = p (detached), pi is clipped to [clip_min, 1].
    """
    ...
    pos_idx_np, js_per_pos, pair_stats = _precompute_pairs_uprl(
    ...
    print(f"[UPRL] Pair stats: {pair_stats}")
    ...
    for epoch in range(num_epochs):
        ...
        for start in iterator:
            ...
            # Pair branch scores
            s_pair_i = model.forward_pair(X_i).squeeze(-1)
            s_pair_j = model.forward_pair(X_j).squeeze(-1)
            base = -F.logsigmoid(s_pair_i - s_pair_j)

            # Point branch predictions on j (detached in correction term)
            s_point_j = model.forward_point(X_j).squeeze(-1)
...
def main():
    ...
    print("=" * 70)
    print("UPRL Reward Modeling (User-wise Pairwise + Fallback Other-User)")
    print("=" * 70)
    print("Loading embeddings and labels from Safetensors file...")

    if not args.binary:
        raise NotImplementedError("UPRL baseline currently supports --binary True only.")
