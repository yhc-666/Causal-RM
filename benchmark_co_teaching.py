...
def loss_coteaching(loss1, loss2, forget_rate, mask, args):
    """
    Co-teaching loss logic.
    """
    # IMPORTANT: We must ignore unobserved data (mask=0).
    loss1[mask == 0] = float('inf')
    loss2[mask == 0] = float('inf')

    ind1_sorted = torch.argsort(loss1.data)
    ind2_sorted = torch.argsort(loss2.data)
    ...
    # Filter out any indices that might still correspond to infinite loss (if batch is mostly empty)
    # This ensures we don't train on masked data even if remember_rate is high
    ind1_update = ind1_update[loss1[ind1_update] != float('inf')]
    ind2_update = ind2_update[loss2[ind2_update] != float('inf')]

    if len(ind2_update) == 0:
        # If Model 2 selected nothing (empty or all masked), Model 1 gets 0 loss
        loss1_mean = torch.tensor(0.0, device=loss1.device, requires_grad=True)
    else:
        loss1_mean = torch.mean(loss1[ind2_update])

    # Model 2 uses samples selected by Model 1
    if len(ind1_update) == 0:
        # If Model 1 selected nothing, Model 2 gets 0 loss
...
def train(net1, net2, train_loader, optimizer1, optimizer2, num_epochs, val_data, patience, args):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y, batch_mask in bar:
            ...
            loss1, loss2 = loss_coteaching(loss1_raw, loss2_raw, rate_schedule[epoch], batch_mask, args)
