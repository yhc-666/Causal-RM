#!/bin/bash
# Grid search script for benchmark_rmf.py (CPU version)
# Usage: bash scripts/grid_search/rmf.sh --alpha 0.5 --dataset hs
...
echo "============================================"
echo "Grid Search for RMF Benchmark"
echo "============================================"
...
for _clip_min in "${_clip_min_list[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="" python -u models_pu/benchmark_rmf.py \
        --desc "$desc" \
        --data_name "$DATASET" \
        --alpha "$ALPHA" \
        --lr "$_lr" \
