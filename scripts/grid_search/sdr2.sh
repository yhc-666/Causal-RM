#!/bin/bash
# Grid search script for benchmark_sdr2.py (CPU version)
# Usage: bash scripts/grid_search/sdr2.sh --alpha 0.5 --dataset hs
...
echo "============================================"
echo "Grid Search for SDR2 Benchmark"
echo "============================================"
...
for _eta in "${_eta_list[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="" python -u models_debias/benchmark_sdr2.py \
        --desc "$desc" \
        --data_name "$DATASET" \
        --alpha "$ALPHA" \
        --lr "$_lr" \
