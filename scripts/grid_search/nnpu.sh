#!/bin/bash
# Grid search script for benchmark_nnpu.py (CPU version)
# Usage: bash scripts/grid_search/nnpu.sh --alpha 0.5 --dataset hs
...
echo "============================================"
echo "Grid Search for NNPU Benchmark"
echo "============================================"
...
for _w_reg in "${_w_reg_list[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="" python -u models_pu/benchmark_nnpu.py \
        --desc "$desc" \
        --data_name "$DATASET" \
        --alpha "$ALPHA" \
        --lr "$_lr" \
