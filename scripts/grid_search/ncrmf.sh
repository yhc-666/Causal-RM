#!/bin/bash
# Grid search script for benchmark_ncrmf.py (CPU version)
# Usage: bash scripts/grid_search/ncrmf.sh --alpha 0.5 --dataset hs
...
echo "============================================"
echo "Grid Search for NCRMF Benchmark"
echo "============================================"
...
for _clip_min in "${_clip_min_list[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="" python -u models_pu/benchmark_ncrmf.py \
        --desc "$desc" \
        --data_name "$DATASET" \
        --alpha "$ALPHA" \
        --lr "$_lr" \
