#!/bin/bash
# Grid search script for benchmark_dr.py (CPU version)
# Usage: bash scripts/grid_search/dr.sh --alpha 0.5 --dataset hs
...
echo "============================================"
echo "Grid Search for DR Benchmark"
echo "============================================"
...
# ============== Grid search ==============
job_number=0
total_combinations=$((${#_lr_list[@]} * ${#_batch_size_list[@]} * ${#_l2_reg_list[@]} * ${#_l2_imp_list[@]} * ${#_w_reg_list[@]} * ${#_w_imp_list[@]} * ${#_clip_min_list[@]}))
...
for _clip_min in "${_clip_min_list[@]}"; do
    ...
    EXP_DIR="${DATASET}_alpha${ALPHA}_lr${_lr}_bs${_batch_size}_l2${_l2_reg}_l2imp${_l2_imp}_wreg${_w_reg}_wimp${_w_imp}_clip${_clip_min}"
    ...
    CUDA_VISIBLE_DEVICES="" python -u models_debias/benchmark_dr.py \
        --desc "$desc" \
        --data_name "$DATASET" \
        --alpha "$ALPHA" \
        --lr "$_lr" \
