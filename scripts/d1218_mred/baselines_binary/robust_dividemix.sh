...
_seed_list=(42)
_p_threshold_list=(0.5)
_alpha_mix_list=(0.5 4)
_lambda_u_list=(10 25)
_num_steps_list=(1)
_perturb_step_list=(0.1 0.2 0.5)
_l2_reg_list=(1e-7 1e-6)
_w_reg_list=(1.0 0.2 2 5)


for _seed in "${_seed_list[@]}"; do
for _batch_size in "${_batch_size_list[@]}"; do
for _w_reg in "${_w_reg_list[@]}"; do
for _p_threshold in "${_p_threshold_list[@]}"; do
for _alpha_mix in "${_alpha_mix_list[@]}"; do
for _lambda_u in "${_lambda_u_list[@]}"; do
for _num_steps in "${_num_steps_list[@]}"; do
for _perturb_step in "${_perturb_step_list[@]}"; do
...
for _alpha in "${_alpha_list[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$gpu_allocation python -u benchmark_robust_dividemix.py $CMD \
