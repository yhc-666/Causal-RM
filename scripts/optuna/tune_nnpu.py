"""
Optuna hyperparameter tuning script for NNPU model.

Optimizes R2 on test with val-based early stopping.
Runs datasets in order: hs, saferlhf, ufb (configurable).
Uses dataset-specific class_prior ranges.

Usage:
    python scripts/optuna/tune_nnpu.py
    python scripts/optuna/tune_nnpu.py --datasets hs,saferlhf,ufb --n_trials 200
    python scripts/optuna/tune_nnpu.py --alpha 0.2 --resume
"""
...
MODEL_NAME = "nnpu"
SCRIPT_PATH = "models_pu/benchmark_nnpu.py"
REPO_ROOT = Path(__file__).resolve().parents[2]

# Dataset-specific class_prior ranges
CLASS_PRIOR_RANGES = {
    "hs": (0.5, 0.8),
    "saferlhf": (0.3, 0.6),
    "ufb": (0.3, 0.7),
...
def _suggest_params(trial: optuna.Trial, data_name: str) -> dict:
    """Suggest hyperparameters for NNPU model."""
    # Get dataset-specific class_prior range
    prior_low, prior_high = CLASS_PRIOR_RANGES.get(data_name, (0.3, 0.8))

    return {
        "lr": float(trial.suggest_float("lr", 1e-5, 3e-3, log=True)),
        "l2_reg": float(trial.suggest_float("l2_reg", 1e-8, 1e-1, log=True)),
        "batch_size": int(trial.suggest_categorical("batch_size", [128, 256, 512, 1024, 2048])),
        "w_reg": float(trial.suggest_categorical("w_reg", [0.005, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        "class_prior": float(trial.suggest_float("class_prior", prior_low, prior_high)),
    }
...
def tune_one_dataset(args) -> dict:
    ...
    prior_range = CLASS_PRIOR_RANGES.get(args.data_name, (0.3, 0.8))
    print("=" * 70)
    print(f"Tuning: {MODEL_NAME}  data={args.data_name}  alpha={args.alpha}")
    print(f"class_prior range: {prior_range}")
