"""
Optuna hyperparameter tuning script for CounterIF model.

Optimizes R2 on test with val-based early stopping.
Runs datasets in order: hs, saferlhf, ufb (configurable).

Usage:
    python scripts/optuna/tune_counterif.py
    python scripts/optuna/tune_counterif.py --datasets hs,saferlhf,ufb --n_trials 300
    python scripts/optuna/tune_counterif.py --alpha 0.2 --resume
"""
...
MODEL_NAME = "counterif"
SCRIPT_PATH = "models_debias_pu/benchmark_counterif.py"
REPO_ROOT = Path(__file__).resolve().parents[2]

# Dataset-specific parameter ranges (based on training set sizes)
# hs: train=28,264 | saferlhf: train=118,251 | ufb: train=97,816
DATASET_PARAM_RANGES = {
    "hs": {
        "ocsvm_batch_size": [1024, 2048, 4096],
        "pair_max_un_he": [2000, 5000, 10000],
        "pair_max_hu_un": [2000, 5000, 10000],
    },
    "saferlhf": {
        "ocsvm_batch_size": [4096, 8192, 16384],
        "pair_max_un_he": [10000, 25000, 50000],
        "pair_max_hu_un": [10000, 25000, 50000],
    },
    "ufb": {
        "ocsvm_batch_size": [4096, 8192, 16384],
        "pair_max_un_he": [10000, 25000, 50000],
...
def _suggest_params(trial: optuna.Trial, data_name: str) -> dict:
    """Suggest hyperparameters for CounterIF model."""
    # Get dataset-specific ranges
    ds_ranges = DATASET_PARAM_RANGES.get(data_name, DATASET_PARAM_RANGES["ufb"])

    return {
        # Common parameters
        "lr": float(trial.suggest_categorical("lr", [1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3])),
        ...
        "batch_size_point": int(trial.suggest_categorical("batch_size_point", [128, 256, 512, 1024, 2048])),
        "batch_size_pair": int(trial.suggest_categorical("batch_size_pair", [128, 256, 512, 1024, 2048])),
        "batch_size_ipm": int(trial.suggest_categorical("batch_size_ipm", [128, 256, 512, 1024])),
        # Loss weights
        "lambda_point": float(trial.suggest_categorical("lambda_point", [0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        "lambda_pair": float(trial.suggest_categorical("lambda_pair", [0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        "lambda_ipm": float(trial.suggest_categorical("lambda_ipm", [0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        # IPM parameters
        "ipm_lam": float(trial.suggest_categorical("ipm_lam", [1.0, 2.0, 5.0, 10.0, 20.0, 50.0])),
        "ipm_its": int(trial.suggest_categorical("ipm_its", [5, 10, 20])),
        "ipm_p": float(trial.suggest_categorical("ipm_p", [0.2, 0.5, 0.8])),
        # OCSVM stratification parameters
        "target_percentile": float(trial.suggest_categorical("target_percentile", [80.0, 85.0, 90.0, 95.0])),
        "hn_percentile": float(trial.suggest_categorical("hn_percentile", [5.0, 10.0, 15.0, 20.0])),
        # Dataset-specific parameters
        "ocsvm_batch_size": int(trial.suggest_categorical("ocsvm_batch_size", ds_ranges["ocsvm_batch_size"])),
        "pair_max_un_he": int(trial.suggest_categorical("pair_max_un_he", ds_ranges["pair_max_un_he"])),
        "pair_max_hu_un": int(trial.suggest_categorical("pair_max_hu_un", ds_ranges["pair_max_hu_un"])),
    }
...
def tune_one_dataset(args) -> dict:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        params = _suggest_params(trial, args.data_name)
    ...
    ds_ranges = DATASET_PARAM_RANGES.get(args.data_name, DATASET_PARAM_RANGES["ufb"])
    print("=" * 70)
    print(f"Tuning: {MODEL_NAME}  data={args.data_name}  alpha={args.alpha}")
    print(f"Dataset-specific ranges: ocsvm_batch_size={ds_ranges['ocsvm_batch_size']}")
    print(f"                         pair_max_un_he={ds_ranges['pair_max_un_he']}")
    print(f"                         pair_max_hu_un={ds_ranges['pair_max_hu_un']}")
