"""
Optuna hyperparameter tuning script for SDR2 model.

Optimizes R2 on test with val-based early stopping.
Runs datasets in order: hs, saferlhf, ufb (configurable).

Usage:
    python scripts/optuna/tune_sdr2.py
    python scripts/optuna/tune_sdr2.py --datasets hs,saferlhf,ufb --n_trials 300
    python scripts/optuna/tune_sdr2.py --alpha 0.2 --resume
"""
...
MODEL_NAME = "sdr2"
SCRIPT_PATH = "models_debias/benchmark_sdr2.py"
...
def _suggest_params(trial: optuna.Trial) -> dict:
    """Suggest hyperparameters for SDR2 model."""
    return {
        ...
        "l2_prop": float(trial.suggest_categorical("l2_prop", [1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2, 1e-1, 2e-1, 5e-1])),
        "l2_imp": float(trial.suggest_categorical("l2_imp", [1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5, 1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2, 1e-1, 2e-1, 5e-1])),
        ...
        "w_prop": float(trial.suggest_categorical("w_prop", [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        "w_imp": float(trial.suggest_categorical("w_imp", [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0])),
        "clip_min": float(trial.suggest_categorical("clip_min", [1e-4, 1e-3, 1e-2, 5e-2, 1e-1, 2e-1])),
        "eta": float(trial.suggest_categorical("eta", [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0])),
    }
