...
from evaluate.evaluator1 import aoa_evaluator, unbiased_evaluator
...
class BISER(AbstractRecommender):
    def __init__(self, sess, data, train, val, test, num_users: np.array, num_items: np.array, \
        ...
        if data == 'coat':
            ...
            self.hidden_dim_u = 400
            self.hidden_dim_i = 400
            self.eta_u = 0.01
            self.eta_i = 0.05
            self.reg_u = 0
            self.reg_i = 0
    ...
    def train_model(self, pscore, unbiased_eval):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch % 1 == 0 and not er_stop_flag:
                ...
                if max_score < val_ret.loc['MAP@3', f'BISER_{dim}']:
                    max_score = val_ret.loc['MAP@3', f'BISER_{dim}']
