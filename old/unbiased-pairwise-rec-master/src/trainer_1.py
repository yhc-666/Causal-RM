...
from PointWise_trainer import pointwise_trainer, cjmf_trainer
...
class Trainer:
    ...
    def __init__(self, data: str, random_state: list, hidden: int, date_now: str, max_iters: int = 1000,
                 lam: float = 1e-4, batch_size: int = 12, wu=0.1, wi=0.1, alpha: float = 0.5, \
                 clip: float = 0.1, eta: float = 0.1, model_name: str = 'mf', unbiased_eval: bool = True,
                 neg_sample: int = 10, C: int = 8, alpha_cjmf: float = 220000, beta_cjmf: float = 0.5, \
                 macr_c: float = 0.1, macr_alpha: float = 0.1, macr_beta: float = 0.1,
                 best_model_save: bool = True) -> None:
    ...
    def run(self) -> None:
        ...
        for random_state in tqdm(self.random_state):
            ...
            if self.model_name in ['uae', 'iae']:
                weights_enc, weights_dec, bias_enc, bias_dec = ae_trainer(sess, data=self.data, train=train, val=val,
                                                                          test=test,
                                                                          num_users=num_users, num_items=num_items,
                                                                          n_components=self.dim,
                                                                          eta=self.eta, lam=self.lam,
                                                                          max_iters=self.max_iters,
                                                                          batch_size=self.batch_size,
                                                                          model_name=self.model_name,
                                                                          item_freq=item_freq,
                                                                          unbiased_eval=self.unbiased_eval,
                ...
                weights_enc_u, weights_dec_u, bias_enc_u, bias_dec_u, weights_enc_i, weights_dec_i, bias_enc_i, bias_dec_i = ae_trainer(
                    sess, data=self.data, train=train, val=val, test=test,
                ...
                u_emb, i_emb = cjmf_trainer(sess=sess, data=self.data, n_components=self.dim, num_users=num_users,
                                            ...
                                            alpha=self.alpha_cjmf, beta=self.beta_cjmf, train=train, val=val,
                                            seed=random_state, model_name=self.model_name)

            elif self.model_name in ['macr']:
                model = MACR(model_name=self.model_name,
                             num_users=num_users, num_items=num_items,
                             dim=self.dim, lam=self.lam, eta=self.eta, batch_size=self.batch_size, c=self.macr_c,
                             alpha=self.macr_alpha, beta=self.macr_beta)
            ...
            if self.best_model_save:
                if self.model_name in ['uae', 'iae']:
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_enc.npy',
                            arr=weights_enc)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_dec.npy',
                            arr=weights_dec)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_enc.npy',
                            arr=bias_enc)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_dec.npy',
                            arr=bias_dec)

                elif self.model_name in ['proposed']:
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_enc_u.npy',
                            arr=weights_enc_u)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_dec_u.npy',
                            arr=weights_dec_u)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_enc_u.npy',
                            arr=bias_enc_u)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_dec_u.npy',
                            arr=bias_dec_u)

                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_enc_i.npy',
                            arr=weights_enc_i)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_weights_dec_i.npy',
                            arr=weights_dec_i)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_enc_i.npy',
                            arr=bias_enc_i)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_bias_dec_i.npy',
                            arr=bias_dec_i)
                else:
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_u_emb.npy',
                            arr=u_emb)
                    np.save(file=ret_path / f'{self.date_now}_dim_{self.dim}_random_{random_state}_i_emb.npy',
                            arr=i_emb)
        ...
        sub_results_mean.to_csv(
            ret_path / f'{self.date_now}_dim_{self.dim}_lr_{self.eta}_reg_{self.lam}_{self.batch_size}_random_{len(self.random_state)}.csv')