...
class Trainer:
    ...
    def __init__(self, data: str, max_iters: int = 1000, batch_size: int = 12,
        ...
        # print(params)
        if model_name not in ['expomf', 'cjmf', 'BISER','Ours']:
    ...
    def run(self, num_sims: int = 10) -> None:
        ...
        if 'bpr' in self.model_name:
            ...
            train = np.load(f'../data/{self.data}_st_1/pair/ours_bak_ocsvm/Ours_train_70_10.npy')
            val = np.load(f'../data/{self.data}_st_1/pair/ours_bak_ocsvm/Ours_val_70_5.npy')
        ...
        for seed in np.arange(num_sims):
            ...
            if 'bpr' in self.model_name:
                ...
                if os.path.exists(labeled_train_path):
                    ...
                    labeled_train_neg = labeled_train[labeled_train[:, 2] == 0 & (labeled_train[:, 3] * labeled_train[:, 4] == 1)] 
                    labeled_train_pairs = []
                    ...
                    np.save(f'/home/zhouchuan/unbiased-pairwise-rec-master/data/{self.data}_st_1/pair/ours_bak_ocsvm/Ours_train_{self.percentile}_{self.HN_percentile}.npy', labeled_train_pairs)
                    # print(labeled_train_pairs.shape)
                    # print(labeled_train_pairs[:10])                    
                    # exit()
                if os.path.exists(labeled_val_path):
                    labeled_val = np.load(labeled_val_path)
                    labeled_val_pos = labeled_val[labeled_val[:, 2] == 1]
                    labeled_val_neg = labeled_val[labeled_val[:, 2] == 0 & (labeled_val[:, 3] * labeled_val[:, 4] == 1)] 
                    labeled_val_pairs = []
                    ...
                    np.save(f'/home/zhouchuan/unbiased-pairwise-rec-master/data/{self.data}_st_1/pair/ours_bak_ocsvm/Ours_val_{self.percentile}_{self.HN_percentile}.npy', labeled_train_pairs)
                    # print(labeled_val_pairs.shape)
                    # print(labeled_val_pairs[:10])                    
                    # exit()                    
                else:
