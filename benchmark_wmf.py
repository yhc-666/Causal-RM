...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/wmf/{pre_args.data_name}",
    ...
    parser.add_argument("--weight", type=float, help="Negative sample weight (WMF)")
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    """
    Train reward model using WMF-style loss adapted from recommender-system RMF.

    WMF sets propensity to 1 for all samples and uses a negative weight:
      L = sum_i [ y_i * (1 - p_i)^2 + w * (1 - y_i) * p_i^2 ] / sum_i [ y_i + w * (1 - y_i) ]

    where p_i = sigmoid(logit) in binary mode.
    """
    ...
    for epoch in range(num_epochs):
        ...
        bar = (
            tqdm(train_loader, desc=f"Training WMF Model Epoch {epoch + 1}/{num_epochs}", leave=False)
            if args.use_tqdm
            else train_loader
        )
        for batch_X, batch_y, batch_mask in bar:
            ...
            loss_per_sample = y * torch.square(1.0 - pred) + w * (1.0 - y) * torch.square(pred)
            denom = torch.sum(y + w * (1.0 - y))
...
def main():
    ...
    print("\n" + "=" * 70)
    print("Step 1: Training WMF Reward Model")
    print("=" * 70)
