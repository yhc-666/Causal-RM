"""
Revised from pot library: fixing device bugs
"""
...
def partial_wasserstein(a, b, M, m=None, nb_dummies=1, log=False, **kwargs):
    ...
    **kwargs : dict
    ...
    M_extended = nx.concatenate(
        (
            nx.concatenate((M, nx.zeros((M.shape[0], M_extension.shape[1]), type_as=M)), axis=1),
            nx.concatenate(
                (nx.zeros((M_extension.shape[0], M.shape[1]), type_as=M), M_extension), axis=1
            ),
        ),
        axis=0,
    )
