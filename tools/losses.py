"""
Adapted from https://github.com/cswjl/eps-softmax/blob/main/losses.py
"""
...
class ECELoss(nn.Module): # Eps-Softmax with CE loss (ECE)
    def __init__(self, m, eps=eps, reduction='mean'):
    ...
    def forward(self, input, target):
        ...
        input = torch.concat([1-input, input], dim=1).clone()
        ...
        loss = F.nll_loss(log_soft_out, target, reduction=self.reduction)
        return loss


class ECEandMAE(nn.Module):
    def __init__(self, m, alpha=1, beta=1, eps=eps, reduction='mean'):
        super(ECEandMAE, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.ece = ECELoss(m, eps=eps, reduction=reduction)
...
class NCELoss(nn.Module):
    def __init__(self, eps=eps, reduction='mean'):
    ...
    def forward(self, pred, labels):
        ...
        probs = torch.concat([1-pred, pred], dim=1)
        probs = torch.clamp(probs, min=self.eps, max=1.0)
        log_probs = torch.log(probs)
        # pred = F.log_softmax(pred, dim=1)
        label_one_hot = F.one_hot(labels, log_probs.shape[1]).float().to(log_probs.device)
        loss = -1 * torch.sum(label_one_hot * log_probs, dim=1) / (-log_probs.sum(dim=1))
...
class MAELoss(nn.Module):
    def __init__(self, scale=2.0, reduction='mean'):
...
class AGCELoss(nn.Module):
    def __init__(self, a=1, q=2, reduction='mean'):
    ...
    def forward(self, pred, labels):
        ...
        probs = torch.concat([1-pred, pred], dim=1)
        # pred = F.softmax(pred, dim=1)
        label_one_hot = F.one_hot(labels, probs.shape[1]).float().to(probs.device)
        p_y = torch.sum(label_one_hot * probs, dim=1)
        loss = ((self.a + 1) ** self.q - torch.pow(self.a + p_y, self.q)) / self.q
        if self.reduction == 'mean':
            return loss.mean()
        elif self.reduction == 'sum':
            return loss.sum()
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta=1., a=3, q=1.5, eps=eps, reduction='mean'):
        super(NCEandAGCE, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.nce = NCELoss(eps=eps, reduction=reduction)
        self.agce = AGCELoss(a=a, q=q, reduction=reduction)

    def forward(self, pred, labels):
        labels = labels.long()
        return self.alpha * self.nce(pred, labels) + self.beta * self.agce(pred, labels)
