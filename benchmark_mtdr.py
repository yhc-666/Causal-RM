...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args):
    ...
    where:
        ...
        - L_imp: Imputation/baseline loss on observed samples
        - L_DR: Doubly robust loss
    """
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y, batch_mask in bar:
            ...
            # # Task 2: Imputation loss L_imp = ℓ(r_baseline(x), y) | mask=1
            observed = batch_mask > 0.5
            loss_imp = torch.tensor(0.0, device=batch_X.device)
            if observed.sum() > 0:
                baseline_pred_obs = baseline_pred[observed]
                loss_imp = criterion_reward(baseline_pred_obs, batch_y[observed])
...
def main():
    ...
    print("="*70)
    print("Multitask Doubly Robust (DR) Reward Modeling")
    print("="*70)
