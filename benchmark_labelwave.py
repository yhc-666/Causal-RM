...
def parse_arguments():
    ...
    base_defaults = {
        "desc": "foo",
        "is_training": True,
        "output_dir": f"./results/cache/labelwave/{pre_args.data_name}",
        "data_root": "../embeddings/biased_noisy",
        "model_name": "FsfairX-LLaMA3-RM-v0.1",
        "estimator_name": "labelwave",
        ...
        "lw_k": 10, # LabelWave parameter: Moving Average window size
        "rerun": False,
        "monitor_on": "loss",
        "binary": True,
        "r10": 0.1,
    ...
    parser.add_argument("--lw_k", type=int, help="Window size for LabelWave moving average") # key parameter
...
def train(model, train_loader, optimizer, num_epochs, val_data, patience, args, train_size, global_mask):
    """
    ...
    pc_history = []
    pc_smoothed_history = []
    best_pc_smoothed = float('inf')
    patience_counter = 0

    criterion = nn.MSELoss(reduction='none') if not args.binary else nn.BCEWithLogitsLoss(reduction='none')

    previous_predictions = torch.zeros(train_size, dtype=torch.long if args.monitor_on == 'acc' else torch.float32)
    current_predictions = torch.zeros(train_size, dtype=torch.long if args.monitor_on == 'acc' else torch.float32)
    valid_indices = torch.nonzero(global_mask.cpu() > 0.5).squeeze()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y, batch_mask, batch_indices in bar:
            optimizer.zero_grad()
            ...
            # --- LabelWave: In-Loop Prediction Tracking ---
            with torch.no_grad():
                if args.monitor_on == 'acc':
                    # Convert reward_pred to binary predictions (0 or 1)
                    if args.binary:
                        batch_preds = (torch.sigmoid(reward_pred) > 0.5).long().cpu()
                    else: # For regression, thresholding or raw comparison needed.
                        batch_preds = F.relu(reward_pred).round().long().cpu()
                else: # For loss, use raw predictions
                    batch_preds = reward_pred.cpu()
                current_predictions[batch_indices] = batch_preds

        # --- Label Wave Step: Calculate Prediction Changes (PC) ---
        pc_value = 0
        pc_smoothed = 0
        if epoch > 0: # We need at least one previous epoch
            # Only calculate changes on OBSERVED samples (mask=1)
            if args.monitor_on == 'acc':
                diff = (current_predictions[valid_indices] != previous_predictions[valid_indices])
                pc_value = diff.sum()
            else: # For loss, use raw predictions
                if args.binary:
                    previous_predictions = F.sigmoid(previous_predictions)
                diff = criterion(current_predictions[valid_indices], previous_predictions[valid_indices])
                pc_value = torch.mean(diff)
            pc_history.append(pc_value.item())

            # Calculate Moving Average (PC')
            if len(pc_history) >= args.lw_k:
                pc_smoothed = sum(pc_history[-args.lw_k:]) / args.lw_k
                if pc_smoothed < best_pc_smoothed:
                    best_pc_smoothed = pc_smoothed
                    patience_counter = 0
                    torch.save(model.state_dict(), f'{args.output_dir}/best_model.pth')
                else:
                    patience_counter += 1
                    if patience_counter >= patience:
                        print(f"LabelWave Early stopping triggered after {epoch + 1} epochs.'")
                        break
            else:
                pc_smoothed = sum(pc_history) / len(pc_history)
            pc_smoothed_history.append(pc_smoothed)

        # Update previous predictions for next epoch comparison
        previous_predictions.copy_(current_predictions)

        val_loss = evaluate(model, val_data, args)
        if epoch % 4 == 0:
            print(f'Epoch {epoch + 1}/{num_epochs}, Train loss: {epoch_loss/len(train_loader):.5f}, Val loss: {val_loss:.5f}, PC: {pc_value:.5f}, PC Smoothed: {pc_smoothed:.5f}')
...
def main():
    ...
    print("="*70)
    print("LabelWave Reward Modeling")
    print("="*70)
    ...
    train_loader = DataLoader(TensorDataset(X_train_full, y_train_full, mask_train.float(), train_indices), batch_size=args.batch_size, shuffle=True)
    ...
    train(
        ...
        args=args,
        train_size=X_train_full.shape[0], # Pass total size for tensor initialization
        global_mask=mask_train # Pass global mask to filter PC calculation
    )
